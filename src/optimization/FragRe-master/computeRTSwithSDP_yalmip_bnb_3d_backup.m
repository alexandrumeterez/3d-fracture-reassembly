...
if ~exist('th','var'), th = 5e-2; end

% bounds on scale parameters
%if ~exist('sLU','var'), sLU = [0.2 5.0]; end
if ~exist('sLU','var'), sLU = [0.2 5.0]; end
...
if ~exist('enfRot','var'), enfRot=0; end
...
lowerS  = sLU(1);
upperS = sLU(2);
% Constraints = [ Constraints, s>=lowerS, s<=upperS ];
% Change FH:
Constraints = [ Constraints, s>=lowerS, s<=upperS ];
...
disp(['s_opt = ',num2str(abs(det(R)^(1/3)))]);
