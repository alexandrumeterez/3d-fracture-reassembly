...
class SDExtractor(object):
	def __init__(self, path):
		...
		# Output dir
		self.output_path = os.path.join(self.path, 'keypoints_cls')
...
if __name__ == '__main__':
	...
	parser.add_argument('--r', default=0.01, type=float)


	args = parser.parse_args()
	data_path = "/home/sombit/object_new/"
	...
	for obj in objects:	
		...
		for obj_list in objects_list:
				print(obj_list)
				path = os.path.join(data_path, obj, obj_list)
				print(path)
				sde = SDExtractor(path)
				sde.extract_keypoints(args.nkeypoints, args.r)
		# sde = SDExtractor(path)
		# sde.extract_keypoints(args.nkeypoints, args.r)
	# extractor = SDExtractor(object_dir)
